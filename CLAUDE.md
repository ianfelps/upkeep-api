...
- `Habit` has `Title` (max 100), `Description` (max 500), `Icon` (max 50), `Color` (max 7, required), `FrequencyType` (enum: Daily=1, Weekly=2, Monthly=3), `TargetValue` (≥1), `IsActive` (bool). Linked to RoutineEvents via `HabitRoutineLink` join table. Create/Update DTOs accept optional `routineEventIds[]` — service replaces links atomically on update. `HabitDto` includes `linkedRoutineEventIds[]`. `GET /habits?updatedSince=` for delta sync.
...
- Enums serialize globally as strings via `JsonStringEnumConverter` (registered in `Program.cs` on `AddControllers().AddJsonOptions`). API requests/responses use the enum name (`"Daily"`, `"Completed"`, `"recurring"`) — never the underlying int. DTOs may expose enums directly; the converter handles wire format. Existing string-typed DTO fields (e.g. `HabitDto.FrequencyType`) still work unchanged.
- Auth uses **access token (short-lived JWT) + refresh token (long-lived, 60 days default)**. Refresh tokens are persisted as SHA-256 hashes in `RefreshTokens` with `RevokedAt`, `FamilyId`, and `ReplacedByTokenId`. `POST /auth/refresh` rotates within the same family (revokes the old, issues a new pair, links via `ReplacedByTokenId`) — never keep an old refresh token alive after refresh. **Reuse detection**: if a presented token is already revoked AND already rotated (`ReplacedByTokenId is not null`), the entire family is revoked via `RevokeFamilyAsync` — treat as theft. `IAuthService.RevokeAllUserTokensAsync(userId)` revokes every active token for a user; called from `UserService` on password change and on email change. `ClockSkew` is 5 min to tolerate device clock drift offline. `Jwt__RefreshExpirationInDays` env var controls refresh lifetime. `Jwt__SecretKey` is validated at startup — must be ≥ 32 UTF-8 bytes or the app throws on boot.
- Sensitive account operations require password re-auth in the body: `PUT /users/me` (`UpdateUserDto.CurrentPassword`), `PATCH /users/me/password` (already), and `DELETE /users/me` (`DeleteAccountDto.CurrentPassword`). Password policy: 8–72 chars (72 is BCrypt's hard limit). `UserService` depends on `IAuthService` to revoke refresh tokens when the password or email changes.
